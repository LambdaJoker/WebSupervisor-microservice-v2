...
| `redis_app_name` | `string` | 变更检测写入 redis_cache 时使用的 `app` 命名空间，如 `web_supervisor`；错误去重缓存 key 也以此为前缀（`<app>:err_dedup:<md5>`） |
| `email_tos` | `[]string` | **监控变化通知**收件人列表（仅发"内容变化"消息），支持 `${ENV}`（示例：`["${QQ_MAIL_ACCOUNT_2667}"]`） |
| `email_tos_admin` | `[]string` | **管理员**收件人列表，用于错误/告警等其它信息。可选，留空则不发送告警邮件（仅记录日志）。支持 `${ENV}` |
| `interval_second` | `number` | 每轮监控的间隔秒数（jobs.json 里的 `intervalSecond` 不生效，以此为准） |

### 错误处理与去重

- **任务级容错**：执行单个任务过程中遇到致命错误（panic，如类型断言失败）不会终止服务，只会跳过本次任务，下一轮继续。仅初始化阶段（配置加载、`stream.Init`、jobs.json 解析）的致命错误才会终止进程。
- **错误缓存与告警**：任务执行中的错误（抓取失败/非 200、解析失败、缓存写入失败、恢复的 panic）会即时写入 Redis 去重缓存。**相同签名（URL+阶段+消息）在 24 小时内重复出现只缓存、不再向管理员发邮件**，避免重复错误刷屏；首次出现的新错误才通过 `email_by_config` 发往 `email_tos_admin`。去重缓存 key：`<redis_app_name>:err_dedup:<md5(signature)>`，TTL 见 `services.errorDedupTTL`。
